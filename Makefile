...
HTTPS_GIT := github.com/fasdfasd/fasfas.git
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=fasfas \
		  -X github.com/cosmos/cosmos-sdk/version.AppName=fasdfsa \
...
ifeq ($(OS),Windows_NT)
	...
	go build -mod=readonly $(BUILD_FLAGS) -o build/fasdfsa ./cmd/fasdfsa
endif

build-windows-client: go.sum
	GOOS=windows GOARCH=amd64 go build -mod=readonly $(BUILD_FLAGS) -o build/fasdfsa.exe ./cmd/fasdfsa
...
install: go.sum
	go install -mod=readonly $(BUILD_FLAGS) ./cmd/fasdfsa
...
draw-deps:
	...
	@goviz -i ./cmd/fasdfsa -d 2 | dot -Tpng -o dependency-graph.png
...
ifeq (,$(shell which heighliner))
	...
	heighliner build -c fasfas --local -f chains.yaml
endif
...
set-testnet-configs:
	fasdfsa config set client chain-id chainid-1
	fasdfsa config set client keyring-backend test
	fasdfsa config set client output text

# import keys from testnet.json into test keyring
setup-testnet-keys:
	-`echo "decorate bright ozone fork gallery riot bus exhaust worth way bone indoor calm squirrel merry zero scheme cotton until shop any excess stage laundry" | fasdfsa keys add acc0 --recover`
	-`echo "wealth flavor believe regret funny network recall kiss grape useless pepper cram hint member few certain unveil rather brick bargain curious require crowd raise" | fasdfsa keys add acc1 --recover`
