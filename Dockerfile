...
# then log output of file /code/bin/fasdfsa
# then ensure static linking
RUN LEDGER_ENABLED=false BUILD_TAGS=muslc LINK_STATICALLY=true make build \
  && file /code/build/fasdfsa \
  && echo "Ensuring binary is statically linked ..." \
  && (file /code/build/fasdfsa | grep "statically linked")

# --------------------------------------------------------
FROM alpine:3.16

COPY --from=go-builder /code/build/fasdfsa /usr/bin/fasdfsa
